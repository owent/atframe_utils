...
class time_utility {
 ...
 public:
   ...
   * @param t 可以指定时间对象
   */
  static ATFRAMEWORK_UTILS_API ATFW_UTIL_SANITIZER_NO_THREAD void update(raw_time_t *t = nullptr) noexcept;

  /**
   * @brief 获取原始系统时间对象
   * @return 最后一次update的时间
   */
  static ATFRAMEWORK_UTILS_API ATFW_UTIL_SANITIZER_NO_THREAD raw_time_t now() noexcept;

  /**
   * @brief 获取当前Unix时间戳
   * @return 最后一次update的时间
   */
  static ATFRAMEWORK_UTILS_API ATFW_UTIL_SANITIZER_NO_THREAD time_t get_now() noexcept;
   ...
   * @return 当前时间的微妙部分
   */
  static ATFRAMEWORK_UTILS_API ATFW_UTIL_SANITIZER_NO_THREAD int32_t get_now_usec() noexcept;
   ...
   * @return 当前时间的微秒部分
   */
  static ATFRAMEWORK_UTILS_API ATFW_UTIL_SANITIZER_NO_THREAD int32_t get_now_nanos() noexcept;

  /**
   * @brief 获取原始系统时间对象,不受set_global_now_offset()影响
   * @return 最后一次update的时间
   */
  static ATFRAMEWORK_UTILS_API ATFW_UTIL_SANITIZER_NO_THREAD raw_time_t sys_now() noexcept;

  /**
   * @brief 获取系统当前Unix时间戳,不受set_global_now_offset()影响
   * @return 最后一次update的时间
   */
  static ATFRAMEWORK_UTILS_API ATFW_UTIL_SANITIZER_NO_THREAD time_t get_sys_now() noexcept;

  /**
   * @brief 获取CPU计数器（用于高性能计算高精度时间）
   * @return CPU计数器
   */
  static ATFRAMEWORK_UTILS_API ATFW_UTIL_SANITIZER_NO_THREAD platform::cpu_time_counter sys_now_cpu_counter() noexcept;

  /**
   * @brief 获取实时的原始系统时间对象,不受set_global_now_offset()影响
   * @return 根据最后一次update和CPU计数器计算
   */
  static ATFRAMEWORK_UTILS_API ATFW_UTIL_SANITIZER_NO_THREAD raw_time_t sys_now_realtime() noexcept;
  ...
  get_global_now_offset() noexcept;

  /**
   * @brief 重置时间的全局偏移（Debug功能）
   * @note 影响now()、get_now()和get_now_usec()的返回结果，还原成真实时间
   */
  static ATFRAMEWORK_UTILS_API ATFW_UTIL_SANITIZER_NO_THREAD void reset_global_now_offset() noexcept;
   ...
   * @return 系统时区的时间偏移
   */
  static ATFRAMEWORK_UTILS_API ATFW_UTIL_SANITIZER_NO_THREAD time_t get_sys_zone_offset() noexcept;

  /**
   * @brief 获取时区时间偏移（如果设置过自定义偏移，使用自定义的值，否则和get_sys_zone_offset()的结果一样)
   * @return 时区的时间偏移
   */
  static ATFRAMEWORK_UTILS_API ATFW_UTIL_SANITIZER_NO_THREAD time_t get_zone_offset() noexcept;
   ...
   */
  static ATFRAMEWORK_UTILS_API ATFW_UTIL_SANITIZER_NO_THREAD void set_zone_offset(time_t t) noexcept;

  /**
   * @brief 获取当前时区今天过了多少秒(不计自定义时区)
   * @return 目前时间的相对今天的偏移
   */
  static ATFRAMEWORK_UTILS_API ATFW_UTIL_SANITIZER_NO_THREAD time_t get_today_now_offset() noexcept;
   ...
   * @return 同一天返回 true
   */
  static ATFRAMEWORK_UTILS_API bool is_same_day(time_t left, time_t right) noexcept;
   ...
   */
  static ATFRAMEWORK_UTILS_API bool is_same_day(time_t left, time_t right, time_t offset) noexcept;
   ...
   */
  static ATFRAMEWORK_UTILS_API bool is_greater_day(time_t left, time_t right) noexcept;
   ...
   * @return right是否和left不是同一天且right大于left返回 true
   */
  static ATFRAMEWORK_UTILS_API bool is_greater_day(time_t left, time_t right, time_t offset) noexcept;
   ...
   */
  static ATFRAMEWORK_UTILS_API time_t get_today_offset(time_t offset) noexcept;
   ...
   * @return 今天0点后offset的时间戳
   */
  static ATFRAMEWORK_UTILS_API time_t get_any_day_offset(time_t checked, time_t offset = 0) noexcept;
   ...
   * @return 时间描述
   */
  static ATFRAMEWORK_UTILS_API raw_time_desc_t get_local_tm(time_t t) noexcept;
   ...
   * @return 时间描述
   */
  static ATFRAMEWORK_UTILS_API raw_time_desc_t get_gmt_tm(time_t t) noexcept;

  /**
   * @brief 判定当前年是否是闰年
   * @return 闰年返回 true
   */
  static ATFRAMEWORK_UTILS_API bool is_leap_year(int year) noexcept;

  /**
   * @brief 判定当前时区时间是否是同一个年
   * @return 同一月返回 true
   */
  static ATFRAMEWORK_UTILS_API bool is_same_year(time_t left, time_t right) noexcept;
   ...
   */
  static ATFRAMEWORK_UTILS_API int get_year_day(time_t t) noexcept;

  /**
   * @brief 判定当前时区时间是否是同一个月
   * @return 同一月返回 true
   */
  static ATFRAMEWORK_UTILS_API bool is_same_month(time_t left, time_t right) noexcept;
   ...
   */
  static ATFRAMEWORK_UTILS_API int get_month_day(time_t t) noexcept;
   ...
   * @return 同一周返回 true
   */
  static ATFRAMEWORK_UTILS_API bool is_same_week(time_t left, time_t right, time_t week_first = 0) noexcept;
   ...
   * @return 不跨同一天的该时间点返回 true
   */
  static ATFRAMEWORK_UTILS_API bool is_same_week_point(time_t left, time_t right, time_t offset,
                                                       time_t week_first = 0) noexcept;
   ...
   */
  static ATFRAMEWORK_UTILS_API int get_week_day(time_t t) noexcept;
   ...
   * @return t所在天的开始时间戳
   */
  static ATFRAMEWORK_UTILS_API time_t get_day_start_time(time_t t = 0) noexcept;
   ...
   * @return t所在周的开始时间戳
   */
  static ATFRAMEWORK_UTILS_API time_t get_week_start_time(time_t t = 0, time_t week_first = 0) noexcept;
   ...
   * @return t所在月的开始时间戳
   */
  static ATFRAMEWORK_UTILS_API time_t get_month_start_time(time_t t = 0) noexcept;
};
}  // namespace time
