...
class log_sink_file_backend {
 public:
  ATFRAMEWORK_UTILS_API log_sink_file_backend();
  ATFRAMEWORK_UTILS_API log_sink_file_backend(const std::string &file_name_pattern);
  ATFRAMEWORK_UTILS_API log_sink_file_backend(const log_sink_file_backend &other);
  ATFRAMEWORK_UTILS_API ~log_sink_file_backend();
 ...
 public:
   ...
   */
  ATFRAMEWORK_UTILS_API void set_file_pattern(const std::string &file_name_pattern);
   ...
   * @see log_formatter::format
   */
  ATFRAMEWORK_UTILS_API void set_writing_alias_pattern(const std::string &file_name_pattern);
   ...
   */
  ATFRAMEWORK_UTILS_API log_sink_file_backend &set_check_interval(time_t check_interval);

  /**
   * @brief 获取定期刷入文件系统的时间间隔
   * @return 定期刷入文件系统的时间间隔(秒)，0则是未启用定期刷入文件系统的功能
   */
  ATFRAMEWORK_UTILS_API time_t get_flush_interval() const;

  /**
   * @brief 设置定期刷入文件系统的时间间隔
   * @param v 定期刷入文件系统的时间间隔(秒)，设为0则是关闭定期刷入文件系统的功能
   */
  ATFRAMEWORK_UTILS_API log_sink_file_backend &set_flush_interval(time_t v);
  ...
  ATFRAMEWORK_UTILS_API log_sink_file_backend &set_max_file_size(size_t max_file_size);

  ATFRAMEWORK_UTILS_API uint32_t get_rotate_size() const;
