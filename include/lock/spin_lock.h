...
namespace util
{
    namespace lock
    {
        ...
        public:
          ...
          void unlock()
          {
              lock_status_.store(static_cast<unsigned int>(UNLOCKED), ::std::memory_order_release);
          }

          bool is_locked()
          {
              return lock_status_.load(::std::memory_order_acquire) == LOCKED;
          }

          bool try_lock()
          {
              return lock_status_.exchange(static_cast<unsigned int>(LOCKED), ::std::memory_order_acq_rel) == UNLOCKED;
          }
          
          bool try_unlock()
          {
              return lock_status_.exchange(static_cast<unsigned int>(UNLOCKED), ::std::memory_order_acq_rel) == LOCKED;
          }
