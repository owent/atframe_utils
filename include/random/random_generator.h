...
class LIBATFRAME_UTILS_API_HEAD_ONLY random_manager_wrapper {
 ...
 public:
   ...
   * @return 获取转储数据块的长度
   */
  inline size_t block_size() const noexcept { return core_.block_size(); }
   ...
   */
  inline bool dump(unsigned char *output, size_t size) const noexcept { return core_.dump(output, size); }
   ...
   */
  inline bool load(const unsigned char *input, size_t size) noexcept { return core_.load(input, size); }
