...
CASE_TEST(crypto_cipher, split_ciphers) {
  ...
  std::string in =
      ...
      "bf-cfb:camellia-128-cfb:camellia-"
...
static const unsigned char aes_test_cfb128_nopadding_pt[3][30] = {
    "hello message 0x00000001,hi 1", "hello message 0x00000002,hi 2", "hello message 0x00000003,hi 3"};
...
#  endif
    ...
    if (!info.key.empty()) {
      CASE_EXPECT_EQ(0, ci.set_key(reinterpret_cast<const unsigned char *>(info.key.c_str()),
                                   static_cast<uint32_t>(info.key.size() * 8)));
    }
    ...
    if (mode & util::crypto::cipher::mode_t::EN_CMODE_ENCRYPT) {
      ...
      do {
        if (!info.iv.empty()) {
          CASE_EXPECT_EQ(0, ci.set_iv(reinterpret_cast<const unsigned char *>(info.iv.c_str()),
                                      static_cast<uint32_t>(info.iv.size())));
        } else {
          ci.clear_iv();
        }
...
#  endif
    ...
    if (mode & util::crypto::cipher::mode_t::EN_CMODE_DECRYPT) {
      ...
      if (0 == dec_res) {
        std::chrono::system_clock::time_point end = std::chrono::system_clock::now();
        double ns_count = (double)std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin).count();
        CASE_MSG_INFO() << "\tCipher: " << info.cipher << " => decrypt " << info.plaintext.size() << " bytes in "
                        << ns_count << "ns." << std::endl;
      } else {
