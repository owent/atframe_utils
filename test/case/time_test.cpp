...
CASE_TEST(time_test, sys_now) {
  ...
  CASE_EXPECT_EQ(now, util::time::time_utility::get_sys_now());
...
CASE_TEST(time_test, today_offset) {
  ...
  loffset = tobj.tm_hour * util::time::time_utility::HOUR_SECONDS +
            tobj.tm_min * util::time::time_utility::MINITE_SECONDS + tobj.tm_sec;
