...
function(project_build_tools_patch_remove_imported_link_interface_libraries TARGET_NAME)
    ...
    if (PROPERTY_NAME)
        ...
        if (PATCH_INNER_ACTIVE)
            set_target_properties(${TARGET_NAME} PROPERTIES ${PROPERTY_NAME} "${PATCH_INNER_LIBS}")
            message(STATUS "Patch Remove: ${PROPERTY_NAME} of ${TARGET_NAME} from ${OLD_LINK_LIBRARIES} to ${PATCH_INNER_LIBS}")
        endif ()
    endif ()
endfunction()

function(project_build_tools_patch_add_imported_link_interface_libraries TARGET_NAME)
    get_target_property(OLD_LINK_LIBRARIES ${TARGET_NAME} INTERFACE_LINK_LIBRARIES)
    set(PROPERTY_NAME "")
    if (OLD_LINK_LIBRARIES)
        set(PROPERTY_NAME "INTERFACE_LINK_LIBRARIES")
    ...
    if (OLD_LINK_LIBRARIES)
        ...
        list(APPEND PATCH_INNER_LIBS ${ARGN})
    else ()
        set(PATCH_INNER_LIBS ${ARGN})
    endif ()
    list(REMOVE_DUPLICATES PATCH_INNER_LIBS)

    if (NOT OLD_LINK_LIBRARIES EQUAL PATCH_INNER_LIBS)
        set_target_properties(${TARGET_NAME} PROPERTIES ${PROPERTY_NAME} "${PATCH_INNER_LIBS}")
        message(STATUS "Patch Add: ${PROPERTY_NAME} of ${TARGET_NAME} from ${OLD_LINK_LIBRARIES} to ${PATCH_INNER_LIBS}")
    endif ()
endfunction()
