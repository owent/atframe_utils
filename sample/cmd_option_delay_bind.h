...
// 延迟绑定初始化
bool delay_print(atfw::util::cli::callback_param par) {
  if (par.get_params_number() > 0) puts(par[0]->to_string());
  return false;
}

void delay_init(atfw::util::cli::callback_param par, atfw::util::cli::cmd_option *stChild) {
...
void delay_bind() {
  puts("延迟初始化子绑定");
  atfw::util::cli::cmd_option::ptr_type f = atfw::util::cli::cmd_option::create();
  std::shared_ptr<atfw::util::cli::cmd_option> pc = atfw::util::cli::cmd_option::create();
