...
void bind_ci_cmd_print(ATFRAMEWORK_UTILS_NAMESPACE_ID::cli::callback_param) {
  puts("ci: do nothing! - free func without parameter\n");
}

void bind_ci_cmd_print2(ATFRAMEWORK_UTILS_NAMESPACE_ID::cli::callback_param par, double d) {
  printf("ci: Free Fun B2 Params Num: %d, d => %lf\n", static_cast<int>(par.get_params_number()), d);
}

void bind_ci_cmd_print3(ATFRAMEWORK_UTILS_NAMESPACE_ID::cli::callback_param par, double d, int i) {
  printf("ci: Free Fun B3 Params Num: %d, i => %d, d => %lf\n", static_cast<int>(par.get_params_number()), i, d);
}

void bind_ci_cmd_init() {
  ATFRAMEWORK_UTILS_NAMESPACE_ID::cli::cmd_option_ci::ptr_type stOpt =
      ATFRAMEWORK_UTILS_NAMESPACE_ID::cli::cmd_option_ci::create();
