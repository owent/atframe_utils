...
while getopts "ab:c:d:e:hlr:tus-" OPTION; do
  case $OPTION in
    ...
    t)
      ...
      ;;
    u)
      CMAKE_OPTIONS="$CMAKE_OPTIONS -DPROJECT_ENABLE_UNITTEST=YES -DBUILD_TESTING=ON"
      ;;
    s)
...
if [[ "x$NINJA_BIN" != "x" ]]; then
  ${CMAKE_BIN[@]} .. -G Ninja -DCMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE $CMAKE_OPTIONS $CMAKE_CLANG_TIDY "$@"
elif [[ "$CHECK_MSYS" == "mingw" ]]; then
  ${CMAKE_BIN[@]} .. -G "MinGW Makefiles" -DCMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE $CMAKE_OPTIONS $CMAKE_CLANG_TIDY "$@"
else
  ${CMAKE_BIN[@]} .. -DCMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE $CMAKE_OPTIONS $CMAKE_CLANG_TIDY "$@"
fi
...
if [[ 1 -eq $CMAKE_CLANG_ANALYZER ]]; then
  ...
  if [[ -z "$CMAKE_CLANG_ANALYZER_PATH" ]]; then
    echo "cd '$SCRIPT_DIR/$BUILD_DIR' && scan-build -o report --html-title='$PROJECT_NAME static analysis' $CMAKE_CLANG_ANALYZER_OPTIONS make -j4"
  else
